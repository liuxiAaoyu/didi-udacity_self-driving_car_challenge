...
from lidarnets.preprocessing import tf_image
...
def distorted_bounding_box_crop(image,
    ...
    with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bboxes]):
        ...
        labels, bboxes, orientations = tfe.bboxes_filter_overlap(labels, bboxes, orientations,
                                                   threshold=BBOX_CROP_OVERLAP,
                                                   assign_negative=False)
        return cropped_image, labels, bboxes, distort_bbox, orientations


def preprocess_for_train2(image, labels, bboxes,
...
def preprocess_for_eval(image, labels, bboxes, orientations,
    ...
    with tf.name_scope(scope):
        ...
        image = tf.concat([t1, t2, t3], axis=2)
        ...
        if difficults is not None:
            ...
            orientations = tf.boolean_mask(orientations, mask)
        # Image data format.
        if data_format == 'NCHW':
            image = tf.transpose(image, perm=(2, 0, 1))
        return image, labels, bboxes, orientations, bbox_img
...
def preprocess_image(image,
    ...
    """
    if is_training:
        return preprocess_for_train(image, labels, bboxes, orientations,
                                    out_shape=out_shape,
                                    data_format=data_format)
    else:
        return preprocess_for_eval(image, labels, bboxes, orientations,
                                   out_shape=out_shape,
                                   data_format=data_format,
                                   **kwargs)
