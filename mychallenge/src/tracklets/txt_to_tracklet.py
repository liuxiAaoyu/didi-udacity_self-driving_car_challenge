...
def main():
    ...
    parser.add_argument('-o', '--outdir', type=str, nargs='?', default='/home/xiaoyu/results/_n/model1',
                        help='Output folder')
    ...
    parser.add_argument('-cfn', '--camera_file_name', type=str, nargs='?', default='ford07',
                        help='camera file')
    parser.add_argument('-if', '--inputfile', type=str, nargs='?', default='/home/xiaoyu/lidar.txt',
    ...
    for cvsfile in cvslist:
        filename =  os.path.splitext(os.path.split(cvsfile)[1])[0]

        camera_df = pd.DataFrame()
        camera_df = pd.read_csv(cvsfile, header=None, names=camera_cols)
        ...
        starttime = camera_df.index[0].value
        endtime = camera_df.index[-1].value
        ...
        for lines in fp.readlines():
            lines = lines.replace("\n", "").split(" ")
            if long(lines[1]) < starttime or long(lines[1]) > endtime:
                continue;
            td = dict()
            td["id"] = (int(lines[0]))
        ...
        tracklet_path = os.path.join(dataset_outdir, filename+'.xml')
        collection.write_xml(tracklet_path)
        print(tracklet_path+" is wirtten")
    print("Finish")
