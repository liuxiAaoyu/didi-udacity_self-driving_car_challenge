...
def ssd_bboxes_select_layer(predictions_layer,
    ...
    o_shape = orientations_layer.shape
    orientations_layer = np.reshape(orientations_layer,
                                     (batch_size, -1, o_shape[-1]))
    ...
    if select_threshold is None or select_threshold == 0:
        ...
        oriens = orientations_layer[make]
    else:
        ...
        oriens = orientations_layer[idxes[:-1]]###############################33333

    return classes, scores, oriens, bboxes
...
def ssd_bboxes_select(predictions_net,
    ...
    for i in range(len(predictions_net)):
        classes, scores, oriens, bboxes = ssd_bboxes_select_layer(
            predictions_net[i], localizations_net[i], orientations_net[i], anchors_net[i],
            select_threshold, img_shape, num_classes, decode)
        l_classes.append(classes)
        l_scores.append(scores)
        l_oriens.append(oriens)
    ...
    oriens = np.concatenate(l_oriens, 0)
...
# =========================================================================== #
def bboxes_sort(classes, scores, oriens, bboxes, top_k=400):
...
def bboxes_nms(classes, scores, oriens, bboxes, nms_threshold=0.45):
    ...
    return classes[idxes], scores[idxes], oriens[idxes], bboxes[idxes]
