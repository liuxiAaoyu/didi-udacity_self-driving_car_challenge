...
def random_flip_left_right_with_ori(image, bboxes, oriens, seed=None):
    """Random flip left-right of an image and its bounding boxes.
    """
    def flip_bboxes(bboxes, oriens):
        """Flip bounding boxes coordinates.
        """
        bboxes = tf.stack([bboxes[:, 0], 1 - bboxes[:, 3],
                           bboxes[:, 2], 1 - bboxes[:, 1]], axis=-1)
        oriens = oriens * -1
        return bboxes, oriens
    ...
    with tf.name_scope('random_flip_left_right'):
        ...
        result = control_flow_ops.cond(mirror_cond,
                                       ...
                                       lambda: image)
        # Flip bboxes.
        bboxes, oriens = control_flow_ops.cond(mirror_cond,
                                       lambda: flip_bboxes(bboxes, oriens),
                                       lambda: (bboxes, oriens))
        return fix_image_flip_shape(image, result), bboxes, oriens
